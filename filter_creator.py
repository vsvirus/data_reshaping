...
for k, v in filters.items():
    ...
    # @J: are we sure that we always have the notation subclass.varname? What if we don't have the dot?
    dot = k.find('.')
    subclass = k[:dot]
    var_name = k[dot+1:]
    ...
    if subclass == 'Basic': # this categories should match the dot notation index of the variables
        if var_name == 'Condiciones_Previas':
            ...
            # @J once this is into the data structure you can change this check calling the self.get_type method that I will implement
            if type(v[0])==bool or type(v[0])==str: # categories and booleans
                for var in v:
                    fil = {var_name:var}
                    lista_bs.append(fil)
                    # @J you are not saving it to super_filtro. Is it right?
...
# This create the filter to "send" to Mongo
